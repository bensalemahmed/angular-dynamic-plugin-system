...
### Added
- **Examples**: Added NgModule dashboard plugin example in `examples/dashboard-plugin/`
...
### Fixed
- **NgModule DI Context**: Fixed dependency injection for NgModule plugins
  - Services declared in plugin NgModule now properly inject into components
  - Added `ngModuleRef` property in PluginMetadata to track NgModuleRef separately
  - Reuse NgModuleRef injector when creating components
- **Version Logging**: Added library version logging on initialization for debugging
